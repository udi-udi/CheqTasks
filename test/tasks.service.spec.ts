...
describe('TasksService', () => {
  ...
  describe('getAllTasks', () => {
    it('returns data and total from the repository with pagination', async () => {
      ...
      const result = await service.getAllTasks({ limit: 20, offset: 0 }, 'user-1');
      expect(result).toEqual({ data: [mockTask], total: 1 });
      expect(mockRepository.findAndCount).toHaveBeenCalledWith({ where: { userId: 'user-1' }, take: 20, skip: 0, order: { createdAt: 'DESC' } });
    });

    it('passes limit and offset to the repository', async () => {
      mockRepository.findAndCount.mockResolvedValue([[], 0]);
      await service.getAllTasks({ limit: 5, offset: 10 }, 'user-1');
      expect(mockRepository.findAndCount).toHaveBeenCalledWith({ where: { userId: 'user-1' }, take: 5, skip: 10, order: { createdAt: 'DESC' } });
    });
    ...
    it('returns 5 results and total of 20 when fetching page 3 of a 20-task dataset', async () => {
      ...
      const result = await service.getAllTasks({ limit: 5, offset: 10 }, 'user-1');
  ...
  describe('createTask', () => {
    it('creates and saves a task', async () => {
      ...
      const result = await service.createTask({ title: 'Test task', description: 'A description' }, 'user-1');
      expect(result).toEqual(mockTask);
      expect(mockRepository.create).toHaveBeenCalledWith({ title: 'Test task', description: 'A description', userId: 'user-1' });
    ...
    it('works without a description', async () => {
      ...
      const result = await service.createTask({ title: 'Test task' }, 'user-1');
    ...
    it('throws InternalServerErrorException when repository save fails', async () => {
      ...
      await expect(service.createTask({ title: 'Test task' }, 'user-1')).rejects.toThrow(InternalServerErrorException);
    });
  ...
  describe('getAllTasks error handling', () => {
    it('throws InternalServerErrorException when repository findAndCount fails', async () => {
      ...
      await expect(service.getAllTasks({ limit: 20, offset: 0 }, 'user-1')).rejects.toThrow(InternalServerErrorException);
    });
